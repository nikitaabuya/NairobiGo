...
class BookingDetailsActivityCleaner : AppCompatActivity() {
    private lateinit var database: DatabaseReference

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_booking_details_cleaner)
        ...
        buttonSubmit.setOnClickListener {
            ...
            val booking = Booking(name, location, date, hours, "housecleaner")

            // Log the booking details
            Log.d("BookingDetails", "Booking: $booking")

            // Save to Firebase under the technician node
            database.child("bookings").child("housecleaner").push().setValue(booking)
